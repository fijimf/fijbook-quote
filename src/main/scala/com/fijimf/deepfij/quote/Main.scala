...
object Main extends IOApp {
  ...
  val resourceConf: Resource[IO, Config] = {
    val free: Config => IO[Unit] = (_: Config) => IO {}
  ...
  private def createTransactor: Resource[IO, HikariTransactor[IO]] = {
    for {
      cf <- resourceConf
      driver: String = cf.getString("fijbook.quote.db.driver")
      url: String = cf.getString("fijbook.quote.db.url")
      user: String = cf.getString("fijbook.quote.db.user")
      password: String = cf.getString("fijbook.quote.db.password")
  ...
  def run(args: List[String]): IO[ExitCode] = {
    transactor.use { xa =>
      for {
        _ <- initDB(xa)
        port <- config.map(_.getInt("fijbook.quote.port"))
        exitCode <- QuoteServer
          .stream[IO](xa, port)
    ...
    xa.configure { dataSource =>
      IO {
        Flyway
          ...
          .locations("classpath:db-quote/migration")
          .baselineOnMigrate(true)
          .table("flyway_schema_history_quote")
          .load()
          .migrate()
      }
    }
